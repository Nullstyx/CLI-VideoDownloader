...
def run_start(cfg):
    ...
    while True:
        res_choice = questionary.select("Select resolution", choices=res_choices).ask()
...
def run_settings(cfg):
    while True:
        ...
        if choice.startswith("Toggle"):
            ...
            raw = questionary.text("Proxy (login:password@ip:port):").ask()
            if raw and raw.strip():
                cfg["PROXY_URL"] = raw.strip()
                save_config(cfg)
        elif choice == "Change download folder":
            path = questionary.text("Download folder path:").ask()
